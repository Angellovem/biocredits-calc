"""
BioCredits Calculation Pipeline - Refactored with Adapter Pattern

This pipeline orchestrates the complete biocredits calculation process using the DataAdapter
interface, allowing easy switching between different data sources (Airtable, PostgreSQL, etc.).
...
from calc_utils import (
    clear_biocredits_tables, download_kml_official, kml_to_shp, load_shp, normalize_shps,
    ...
    project_buffer_areas, project_buffer_credits, plot_project_credits
)
...
def run_biocredits_pipeline(adapter):
    """
    Run the complete biocredits calculation pipeline.
    
    Args:
        adapter: DataAdapter instance (e.g., AirtableAdapter, PostgresAdapter, etc.)
    """
    try:
        colombia_tz = pytz.timezone('America/Bogota')
        start_str = datetime.now(colombia_tz).strftime('%Y-%m-%d %H:%M:%S')
        
        # Clear tables using adapter
        clear_biocredits_tables(adapter, ["Logs", "Observations", "Monthly Attribution", "Cummulative Attribution"])
        
        # Download KMLs and metadata using adapter
        insert_log_entry(adapter, 'Start time', start_str)
        land_metadata = download_kml_official(adapter)
        
        # KML to SHP (geographic processing - independent of data source)
        kml_to_shp(source_directory='KML/', destination_directory='SHP/', 
                   original_shp_directory='SHPoriginal/', verbose=True, adapter=adapter)
        kml_to_shp(source_directory='credit_subtypes/KML/', destination_directory='credit_subtypes/SHP/', 
                   original_shp_directory=None, verbose=True, adapter=adapter)

        shp = load_shp('SHP/')
        insert_log_entry(adapter, 'Number of fincas', str(len(shp)))

        normalized_shapes = normalize_shps(shp, logs=True, adapter=adapter)
        ...
        insert_log_entry(adapter, 'Plots with value (platinum, gold):', 
                        upload_to_gcs('biocredits-calc', 'plots_value.html', 'plots_value.html'))

        value_counts = value_lands.groupby('plot_id').agg({'value': 'unique'})
        value_counts['value_str'] = value_counts['value'].apply(lambda v: ' & '.join(v))
        insert_log_entry(adapter, 'Plots with value types:', 
                        str(value_counts['value_str'].value_counts().to_dict()))

        # Download observations using adapter
        records = download_observations(adapter)
        records = observations_to_circles(records, default_crs=4326, buffer_crs=6262)

        insert_gdf_to_airtable(adapter, records, 'Observations', insert_geo=False, delete_all=True)
        ...
        insert_log_entry(adapter, 'Time slider plot (since 2022):', 
                        upload_to_gcs('biocredits-calc', 'plots_slider.html', 'plots_slider.html'))
        ...
        insert_log_entry(adapter, 'Daily Attribution rows:', str(len(attribution)))
        attribution.to_csv('daily_attribution.csv')
        insert_log_entry(adapter, 'Daily attribution csv:', 
                        upload_to_gcs('biocredits-calc', 'daily_attribution.csv', 'daily_attribution.csv'))
        ...
        insert_log_entry(adapter, 'Monthly Attribution rows:', str(len(attr_month)))
        attr_month.to_csv('monthly_attribution.csv')
        insert_log_entry(adapter, 'Monthly attribution csv:', 
                        upload_to_gcs('biocredits-calc', 'monthly_attribution.csv', 'monthly_attribution.csv'))

        attr_cumm = transform_one_row_per_value(attr_cumm, 'cumm')
        insert_log_entry(adapter, 'Cummulative Attribution rows:', str(len(attr_cumm)))
        attr_cumm.to_csv('cummulative_attribution.csv')
        insert_log_entry(adapter, 'Cummulative attribution csv:', 
                        upload_to_gcs('biocredits-calc', 'cummulative_attribution.csv', 'cummulative_attribution.csv'))
        ...
        insert_log_entry(adapter, 'Project credits csv:', 
                        upload_to_gcs('biocredits-calc', 'project_credits.csv', 'project_credits.csv'))
        ...
        for project_biodiversity in sorted(project_credits['project_biodiversity'].unique().tolist()):
            ...
            insert_log_entry(adapter, f'Leakage Report credits plot project: {project_biodiversity}:', 
                           upload_to_gcs('biocredits-calc', f'leakage_report_credits_{project_biodiversity}.html', 
                                       f'leakage_report_credits_{project_biodiversity}.html'))
            insert_log_entry(adapter, f'Leakage Report ratio project: {project_biodiversity}:', 
                           upload_to_gcs('biocredits-calc', f'leakage_report_ratio_{project_biodiversity}.html', 
                                       f'leakage_report_ratio_{project_biodiversity}.html'))
        ...
        for project_biodiversity in sorted(project_credits['project_biodiversity'].unique().tolist()):
            ...
            insert_log_entry(adapter, f'Raindrops Video for project {project_biodiversity}:', 
                           upload_to_gcs('biocredits-calc', video_title, video_title))
        
        # Upload results using adapter
        insert_gdf_to_airtable(adapter, attr_cumm.drop(columns=['proportion_certified']), 
                              'Cummulative Attribution', insert_geo=False, delete_all=True)
        insert_gdf_to_airtable(adapter, attr_month.drop(columns=['proportion_certified']), 
                              'Monthly Attribution', insert_geo=False, delete_all=True)

        end_str = datetime.now(colombia_tz).strftime('%Y-%m-%d %H:%M:%S')
        insert_log_entry(adapter, 'End time', end_str)
        
    except Exception as e:
        error_traceback = traceback.format_exc()
        insert_log_entry(adapter, 'Error', str(e)) 
        insert_log_entry(adapter, "Type of exception:", type(e).__name__)
        insert_log_entry(adapter, "Traceback", error_traceback)
        print('Error', str(e))
...
if __name__ == "__main__":
    ...
    print("Starting BioCredits calculation pipeline...")
